...
function Sidebar() {
  ...
  return (
    <aside className="w-64 p-4 flex flex-col gap-1" style={{ 
      backgroundColor: isDark ? '#0f0f0f' : '#F1F0EE', 
      height: '100vh', 
      position: 'sticky', 
      top: 0 
    }}>
      <div className="mb-6 px-3 pt-3">
        <h1 className="font-bold text-lg" style={{ color: isDark ? '#e2e8f0' : '#37352F' }}>FP Tracker</h1>
        <p className="text-xs mt-0.5" style={{ color: isDark ? '#94a3b8' : '#9B9A97' }}>Freelancer CRM</p>
      </div>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '16px', borderTop: `1px solid ${isDark ? '#2d2d3d' : '#E9E9E7'}` }}>
        ...
        <style>{`
          ...
          .btn-color-mode-switch > label.btn-color-mode-switch-inner { margin: 0; width: 110px; height: 28px; background: #E0E0E0; border-radius: 26px; overflow: hidden; position: relative; transition: all 0.3s ease; display: block; cursor: pointer; }
          .btn-color-mode-switch > label.btn-color-mode-switch-inner:before { content: attr(data-on); position: absolute; font-size: 11px; font-weight: 500; top: 6px; right: 14px; color: #fff; }
          ...
          .btn-color-mode-switch input[type="checkbox"]:checked + label.btn-color-mode-switch-inner:after { content: attr(data-on); left: 58px; background: #3c3c3c; color: #fff; }
        ...
        >
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: isDark ? '#94a3b8' : '#9B9A97' }}>View Profile</p>
          </div>
        </NavLink>
      </div>
    </aside>
